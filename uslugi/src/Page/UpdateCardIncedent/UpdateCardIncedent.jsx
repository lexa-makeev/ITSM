...
function UpdateCardIncedent(props) {
  ...
  const [valueOtvetsvID, setValueOtvetsvID] = useState(null);
  const [valueUslugaID, setValueUslugaID] = useState(null);
  useEffect(() => {
    let FormatData = new FormData();
    FormatData.append("email", localStorage.getItem("email"));
    ...
    axios({
      ...
      .then(function (response) {
        ...
        setValueTopic(data.topic);
        setValueType(data.type);
        setValueStatus(data.status);
        setValueEndTime(data.end_date);
        setValueOpis(data.opis);
        // setValueName(data.opis.fam + "" + )
        setValueOtvetsv(data.id_otv);
        setValueOtvetsvID(data.fam + " " + data.name + " " + data.otch);
        setValueUsluga(data.id_usluga);
        setValueUslugaID(data.components);
      })
      .catch(function () {
        console.log("Ошибка");
      });
  ...
  function onSubmit(e) {
    ...
    axios({
      method: "post",
      url: "http://localhost:80/uslugi/api/UpdateIncedentAll.php",
      data: formData,
      headers: { "Content-Type": "multipart/form-data" },
    })
      .then(function (response) {
            ...
            <div className="info_kratk">
              ...
              <p>
                ...
                <select
                  ...
                  value={valueUsluga}
                >
                  <option value={valueUsluga}>{valueUslugaID}</option>
                  {valueCatalog !== null &&
              ...
              <p>
                ...
                <select
                  ...
                  value={valueStatus}
                >
                  <option value={valueStatus}>{valueStatus}</option>
                  <option value="Новая">Новая</option>
                  <option value="Новая">В работе</option>
                  <option value="Новая">Выполнена</option>
                </select>
              </p>
              <p>
                Время завершения:
...
export default UpdateCardIncedent;
